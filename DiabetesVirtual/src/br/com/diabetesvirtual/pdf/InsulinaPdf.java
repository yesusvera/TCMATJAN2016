...
public class InsulinaPdf {
	...
	public InsulinaPdf(Context ctx,String name,List<Insulina> a) {
		lista = a;
		context = ctx;
		nome = name;
	}
		...
		new Thread() {
			...
			public void run() {
				try {
			        ...
			        c.setPadding(3);
			        tabela.addCell(c);		        
			        c = new PdfPCell(new Phrase("QUANTIDADE",font));
			        c.setHorizontalAlignment(Element.ALIGN_CENTER);
			        c.setBackgroundColor(BaseColor.LIGHT_GRAY);
			        c.setPadding(3);
			        tabela.addCell(c);			        
			        ...
			        for (int i = 0; i < lista.size(); i++) {
						...
						tabela.addCell(lista.get(i).getQtd()+" UI");
