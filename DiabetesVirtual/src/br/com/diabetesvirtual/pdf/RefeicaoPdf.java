...
public class RefeicaoPdf {
	...
	public RefeicaoPdf(Context ctx,String name,List<Refeicao> a) {
		lista = a;
		context = ctx;
		nome = name;
	}
	
	public RefeicaoPdf(Context ctx,String name,int inicio, int fim,List<Refeicao> a) {
		lista = a;
		context = ctx;
		nome = name;
		this.inicio = inicio;
		...
		new Thread() {
			...
			public void run() {
				try {
			        ...
			        c.setPadding(3);
			        tabela.addCell(c);
			        c = new PdfPCell(new Phrase("CARBOIDRATO",font));
			        c.setHorizontalAlignment(Element.ALIGN_CENTER);
			        c.setBackgroundColor(BaseColor.LIGHT_GRAY);
			        c.setPadding(3);
			        tabela.addCell(c);
			        ...
			        document.add(titulo);
			        if (inicio>-1 && fim>-1) {
			        	titulo = new Paragraph("Variação entre "+inicio+"g a "+fim+"g",font_subtitulo);
				        titulo.setAlignment(Element.ALIGN_CENTER);
			        	titulo.add(new Paragraph(" "));
				        document.add(titulo);
					}			        
			        for (int i = 0; i < lista.size(); i++) {
						...
						tabela.addCell(Formatos.formataDouble(lista.get(i).getCarboidrato())+"g");
